...
if (runtype == 'site'):
    sites = 'US-Mj1'           #Site name, list of site names, or 'all' for all sites in site group
    sitegroup = 'Crops'       #Sites defined in <inputdata>/lnd/clm2/PTCLM/<sitegroup>_sitedata.txt
    numproc = 1
else:
...
use_crop       = True      #Use crop model

nyears_ad      =  200      #number of years for ad spinup
nyears_final   =  200      #number of years for final spinup OR for SP run
...
parm_list      = 'parm_list_crops'  #'parm_list_example' #Set parameter list (leave blank for no ensemble)
...
if (runtype == 'site'):
  ...
  if (sites[0] != ''):
    siteinfo = get_site_info(inputdata, sitegroup=sitegroup, use_crop=use_crop)
    if sites[0] == 'all':
...
if (use_SP):
  ...
  if (nyears_ad > 0):
    compsets.append(compset_type+'1850'+compset_base)  #ad_spinup

...
def create_ensemble_script(self):
    ...
    partition = self.slurm_partition() if hasattr(self, 'slurm_partition') else getattr(self, 'partition', '')
    if partition != '':
        parsed_sbatch['partition'] = partition
  ...
  for p in self.ensemble_parms:
    if ('INI' in p):
      ...
      if (p in CNP_parms):
         ...
         myfile = pftfile
      is_fates_json = ('fates' in p and self.fates_param_type == 'json')
      if is_fates_json:
